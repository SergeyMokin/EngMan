...
@Component({
  ...
  templateUrl: './translate-word-page.component.html'
})
export class TranslateWordPageComponent implements OnInit {
  ...
  private Check(): void
  {
    ...
    resAnsw.Original = this.AnswerTask;
    resAnsw.Category = this.Task.Category;
    resAnsw.WordId = this.Task.WordId;
    resAnsw.Translate = this.Task.Word;
    this.apiService.CheckTheAnswerOfWordTask(resAnsw, false).subscribe(
      obj => 
      {
        if(obj ===true)
        {
  ...
  private Next(): void
  {
    ...
    document.getElementById("check-btn").className = "btn";
    this.apiService.GetWordTask(this.ChoosenCategory, this.Indexes, false).subscribe(
      obj => 
      {
        this.Task = obj;
